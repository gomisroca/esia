...
function CardContent({ children, image, name }: { children: ReactNode; image?: string; name: string }) {
  ...
  return (
    <div
      ...
      <div
        className={`absolute inset-0 flex items-center justify-center rounded-sm bg-black/60 py-4 transition-opacity duration-200 ease-in ${isFlipped ? 'opacity-100' : 'opacity-0'} group-hover:opacity-100`}>
        <div className="m-4 flex h-full w-full cursor-default flex-col items-center justify-evenly rounded-sm bg-white/60 p-4 text-center drop-shadow-md transition-all duration-200 dark:bg-black/60">
          {children}
        </div>
      </div>
    </div>
